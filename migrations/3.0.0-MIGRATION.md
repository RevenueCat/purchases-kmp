...
## Explicitly add Amazon module if needed
If you're using the Amazon Appstore, you now need to explicitly add the Amazon module to your project. Add the coordinates to your `libs.versions.toml` file:
```toml
purchases-amazon = { module = "com.revenuecat.purchases:purchases-store-amazon", version = "x.y.z"}
```

And add the dependency to your `build.gradle[.kts]` file:
```kotlin
kotlin { 
  sourceSets {
...
You can find more information in our [documentation](https://www.revenuecat.com/docs/getting-started/installation/android#amazon).
