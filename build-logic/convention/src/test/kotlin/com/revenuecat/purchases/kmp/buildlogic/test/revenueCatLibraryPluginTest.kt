...
class RevenueCatLibraryPluginTestContext(
    ...
    /** Maps target name → subproject name, populated by [SubprojectContext.useSwiftPackage]. */
    internal val targetToSubproject = mutableMapOf<String, String>()
    /** All multi-target packages added via [addMultiTargetSwiftPackage]. */
    internal val multiTargetPackages = mutableListOf<MultiTargetPackageHandle>()
    ...
    fun addMultiTargetSwiftPackage(
        ...
        builder.configure()
        return builder.build().also { multiTargetPackages.add(it) }
    }
    ...
    fun runBuild(vararg arguments: String, expectFailure: Boolean = false): BuildResult {
    ...
    private fun writeSubprojectBuildFile(subproject: SubprojectContext) {
        ...
        val evaluationDependsOnLines = subproject.deriveEvaluationDependencies()
...
class SubprojectContext internal constructor(
    val name: String,
    val dir: File,
    private val context: RevenueCatLibraryPluginTestContext,
) {
    internal val swiftPackageConfigs = mutableListOf<SubprojectSwiftPackageConfig>()

    /**
     * Derives the `evaluationDependsOn` calls needed for this subproject by looking up which
...
class MultiTargetPackageBuilder internal constructor(
    ...
    internal fun build(): MultiTargetPackageHandle {
        ...
        val dependencyGraph = targetSpecs.associate { it.name to it.dependencies }
        return MultiTargetPackageHandle(packageDir = packageDir, targets = targetHandles, dependencyGraph = dependencyGraph)
    }
}
